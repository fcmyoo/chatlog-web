...
class AIService {
   ...
   * 获取或创建AI模型实例
   */
  async getModel(provider, config) {
   ...
   * 调用AI进行分析
   */
  async callAI(prompt, systemPrompt, retryCount = 0) {
    const maxRetries = 3;
    ...
    try {
      ...
      console.error(`❌ AI调用失败 (第${retryCount + 1}次):`, error.message);
