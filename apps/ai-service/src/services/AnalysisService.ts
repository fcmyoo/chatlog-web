...
// 导入类型定义
import type { AnalysisMetadata, AnalysisResult, AnalysisHistoryItem, PaginatedResult, ChatMessage, AnalysisType } from '../types/index.js';
...
class AnalysisService {
   ...
   * 执行AI分析
   */
  async performAnalysis({ groupName, analysisType, customPrompt, timeRange }: AnalysisParams): Promise<AnalysisResult> {
    try {
   ...
   * 生成分析提示词
   */
  generateAnalysisPrompt(analysisType: AnalysisType, chatData: ChatMessage[], customPrompt: string = ''): string {
...
请确保分析内容专业、准确、有价值。`;
   ...
   * 获取分析类型标题
   */
  getAnalysisTitle(analysisType: AnalysisType): string {
    const titles = {
      programming: '编程技术分析',
      science: '科学学习分析', 
      reading: '阅读讨论分析',
   ...
   * 生成预览
   */
  generatePreview(analysisResult: string): string {
   ...
   * 保存分析历史
   */
  async saveAnalysisHistory(metadata: AnalysisMetadata, content: string): Promise<string> {
   ...
   * 获取分析历史列表
   */
  async getAnalysisHistory({ page = 1, pageSize = 10, analysisType, groupName }: HistoryQueryParams): Promise<PaginatedResult<AnalysisHistoryItem>> {
    try {
      const files = await fs.readdir(this.historyDir);
      const jsonFiles = files.filter(file => file.endsWith('.json'));
      
      let histories: AnalysisHistoryItem[] = [];
   ...
   * 预加载常用数据
   */
  async preloadCommonData(groupNames: string[], timeRange?: string): Promise<void> {
   ...
   * 检查存储健康状态
   */
  private async _checkStorageHealth(): Promise<any> {
    try {
      const storageDir = this.historyDir;
      const test文件 = path.join(storageDir, 'test.json');
      await fs.writeJSON(test文件, { test: true });
      await fs.remove(test文件);
