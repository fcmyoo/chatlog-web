...
class GoogleAdapter extends BaseAdapter {
  ...
  async initialize() {
    ...
    this.apiClient = axios.create({
      baseURL: this.config.endpoint || 'https://generativelanguage.googleapis.com/v1beta',
      headers: {
    ...
    console.log(`✅ Google适配器初始化完成: ${this.model}`);
  }
  ...
  buildChatRequest(messages, options) {
    return {
      ...
      max_tokens: options.maxTokens || this.config.maxTokens || 32768,
  ...
  async healthCheck() {
    try {
      ...
      return {
        healthy: false,
        message: `Google适配器健康检查失败: ${error.message}`,
        details: {
          error: error.message,
          model: this.model,
          endpoint: this.config.endpoint,
